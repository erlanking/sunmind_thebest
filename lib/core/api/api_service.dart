...
class ApiService {
  ...
  static const String _baseUrlFromEnvironment = String.fromEnvironment(
    'API_BASE_URL',
    defaultValue: 'https://sunmindthebestbackend-production.up.railway.app',
  );
  ...
  static bool isOfflineError(Object error) {
    ...
    // Only treat as "no internet" when DNS or network stack truly fails.
    // Timeouts and connection-refused mean the server is slow/down —
    // internet can still be present, so do NOT include them here.
    return message.contains('failed host lookup') ||
        message.contains('network is unreachable') ||
        message.contains('no address associated') ||
        message.contains('no route to host') ||
        (message.contains('socket') &&
            (message.contains('network') || message.contains('lookup')));
  }
  ...
  Future<Map<String, dynamic>> lookupDevice(String deviceId) async {
    final res = await _request('/api/devices/lookup/$deviceId');
    return jsonDecode(res.body) as Map<String, dynamic>;
  }
  ...
  Future<void> setDeviceMode(String deviceId, {required String mode}) async {
    await _request(
      '/api/devices/$deviceId/control',
      method: 'POST',
      body: jsonEncode({'mode': mode}),
  ...
  Future<List<Map<String, dynamic>>> getDeviceErrors(String deviceId) async {
    final res = await _request('/api/devices/$deviceId/errors');
    return _decodeListResponse(res.body, endpoint: '/api/devices/$deviceId/errors');
  }

  Future<Map<String, dynamic>> resolveDeviceError(
    String deviceId,
    int errorId,
  ) async {
    final res = await _request(
      '/api/devices/$deviceId/errors/$errorId/resolve',
      method: 'PATCH',
    );
    return jsonDecode(res.body) as Map<String, dynamic>;
  }
  ...
  Future<List<Map<String, dynamic>>> getMaintenanceHistory(
    ...
    final res = await _request('/api/devices/$deviceId/maintenance');
    return _decodeListResponse(
      res.body,
      endpoint: '/api/devices/$deviceId/maintenance',
    );
  }
  ...
  Future<Map<String, dynamic>> recordMaintenance(
    ...
    throw Exception('Некорректный ответ /maintenance');
  }

  Future<void> deleteMaintenance(String deviceId, int maintenanceId) async {
    await _request(
      '/api/devices/$deviceId/maintenance/$maintenanceId',
      method: 'DELETE',
    );
  }

  Future<Map<String, dynamic>> setNightGuardSchedule(
    String deviceId, {
    ...
    final res = await _request(
      '/api/devices/$deviceId/night-guard-schedule',
      method: 'PATCH',
      body: jsonEncode({
        'startHour': startHour,
    ...
    if (decoded is Map<String, dynamic>) return decoded;
    if (decoded is Map) return decoded.cast<String, dynamic>();
    throw Exception('Некорректный ответ /night-guard-schedule');
  }

  Future<Map<String, dynamic>> setCharging(
    String deviceId, {
    required bool isCharging,
  }) async {
    final res = await _request(
      '/api/devices/$deviceId/charge',
      method: 'PATCH',
      body: jsonEncode({'isCharging': isCharging}),
    );
    final decoded = jsonDecode(res.body);
    if (decoded is Map<String, dynamic>) return decoded;
    if (decoded is Map) return decoded.cast<String, dynamic>();
    throw Exception('Некорректный ответ /charge');
  }

  Future<Map<String, dynamic>> setPowerSource(
    String deviceId, {
    required String powerSource, // 'battery' | 'ac'
  }) async {
    final res = await _request(
      '/api/devices/$deviceId/power-source',
      method: 'PATCH',
      body: jsonEncode({'powerSource': powerSource}),
    );
    final decoded = jsonDecode(res.body);
    if (decoded is Map<String, dynamic>) return decoded;
    if (decoded is Map) return decoded.cast<String, dynamic>();
    throw Exception('Некорректный ответ /power-source');
  }

  Future<Map<String, dynamic>> setBatteryChargeMode(
    String deviceId, {
    required String mode, // 'manual' | 'auto'
    required int lowThreshold,
    required int fullThreshold,
    required bool autoSolar,
  }) async {
    final res = await _request(
      '/api/devices/$deviceId/battery-charge-mode',
      method: 'PATCH',
      body: jsonEncode({
        'chargeMode': mode,
    ...
    if (decoded is Map<String, dynamic>) return decoded;
    if (decoded is Map) return decoded.cast<String, dynamic>();
    throw Exception('Некорректный ответ /battery-charge-mode');
  }

  Future<Map<String, dynamic>> setNightGuard(
    String deviceId, {
    required bool enabled,
  }) async {
    final res = await _request(
      '/api/devices/$deviceId/night-guard',
      method: 'PATCH',
    ...
    if (decoded is Map<String, dynamic>) return decoded;
    if (decoded is Map) return decoded.cast<String, dynamic>();
    throw Exception('Некорректный ответ /night-guard');
  }
