...
class _ScanDeviceScreenState extends State<ScanDeviceScreen> {
  ...
  Future<void> _onDetect(BarcodeCapture capture) async {
    if (!scanning || _checking || capture.barcodes.isEmpty) return;
    final code = capture.barcodes.first.rawValue;
    if (code == null) return;
    final parsed = _parseCode(code);
    if (parsed == null) {
      setState(() => error = 'Неверный формат QR');
      return;
    }
    ...
    try {
      final info = await _api.lookupDevice(parsed);
      if (!mounted) return;
      if (info['found'] != true) {
        setState(() {
          error = 'Устройство ещё не подключилось к серверу. Включите ESP и подождите.';
          scanning = true;
          _checking = false;
        });
        return;
      ...
      if (!mounted) return;
      setState(() {
        error = 'Не удалось проверить устройство. Проверьте подключение.';
        scanning = true;
        _checking = false;
      });
      return;
